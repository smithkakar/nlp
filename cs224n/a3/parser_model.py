...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(
            torch.Tensor(self.n_features * self.embed_size, self.hidden_size))
        ...
        self.hidden_to_logits_weight = nn.Parameter(
            torch.Tensor(self.hidden_size, self.n_classes))
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)
        self.hidden_to_logits_bias = nn.Parameter(
            torch.Tensor(self.n_classes))
    ...
    def embedding_lookup(self, w):
        ...
        # Flatten w to combine lookups from all batches
        x = torch.index_select(self.embeddings, 0, w.view(-1))
        # Unflatten x to turn it to batches
        x = x.view(w.size()[0], -1)
        # END YOUR CODE
        return x
...
if __name__ == "__main__":
    ...
    def check_embedding():
        ...
        assert np.all(selected.data.numpy() ==
                      0), "The result of embedding lookup: " \
